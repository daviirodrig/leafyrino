...
const QMap<QString, Type> MESSAGE_TYPING_CONTEXT{
    ...
    {"flags.repeated_message", Type::Bool},
    {"flags.repeated_messages", Type::Bool},
    {"flags.watch_streak", Type::Bool},
    {"flags.webchat_detected", Type::Bool},
    {"message.content", Type::String},
    {"message.length", Type::Int},
    {"moltorino.client_detection", Type::String},
...
ContextMap buildContextMap(const MessagePtr &m, chatterino::Channel *channel)
{
    ...
    ContextMap vars = {
        ...
        {"flags.repeated_message", m->flags.has(MessageFlag::RepeatedMessage)},
        {"flags.repeated_messages", m->flags.has(MessageFlag::RepeatedMessage)},
        {"flags.watch_streak", m->flags.has(MessageFlag::WatchStreak)},
        {"flags.webchat_detected",
         m->clientDetection == Message::ClientDetectionStatus::Web},

        {"message.content", m->messageText},
        {"message.length", m->messageText.length()},
        {"moltorino.client_detection",
         Message::clientDetectionStatusToString(m->clientDetection)},
    };
    {
